  ...
  modal.innerHTML = `
    <div style="background:#030712;border:1px solid #1f2937;border-radius:8px;max-width:900px;width:100%;max-height:90vh;overflow-y:auto;padding:24px;color:#f3f4f6;font-family:monospace;">
      ...
      <div style="margin-top:20px;border-top:1px solid #1f2937;padding-top:16px;">
        ${isPDF ? `<div><iframe src="https://mozilla.github.io/pdf.js/web/viewer.html?file=${encodeURIComponent(absoluteFileUrl)}" style="width:100%;height:500px;border:1px solid #374151;border-radius:4px;"></iframe><p style="margin-top:8px;text-align:center;"><a href="${fileUrl.replace(/"/g, '%22')}" target="_blank" style="color:#3b82f6;font-family:monospace;font-size:0.78rem;">↗ Open PDF directly</a></p></div>` : ['doc','docx','xls','xlsx','ppt','pptx'].includes(ext) ? `<div><iframe src="https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(fileUrl)}" style="width:100%;height:500px;border:1px solid #374151;border-radius:4px;"></iframe><p style="margin-top:8px;text-align:center;"><a href="${fileUrl.replace(/"/g, '%22')}" target="_blank" style="color:#3b82f6;font-family:monospace;font-size:0.78rem;">↗ Open document directly</a></p></div>` : isImage ? `<img src="${fileUrl}" style="max-width:100%;max-height:400px;border-radius:4px;border:1px solid #374151;">` : `<a href="${fileUrl.replace(/"/g, '%22')}" target="_blank" style="color:#3b82f6;text-decoration:underline;">Open ${filename}</a>`}
      </div>
    </div>
  `;
