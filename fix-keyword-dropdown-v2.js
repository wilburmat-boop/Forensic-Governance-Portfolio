...
// 3. Insert new floating dropdown right after the Keywords button tag (small, precise anchor)
const buttonAnchor = '>Keywords ▾</button>';
const floatingDropdown = '>Keywords ▾</button>\n      <div id="keyword-dropdown" style="position:absolute;top:100%;right:0;z-index:1001;display:none;margin-top:4px;padding:12px;background:#111827;border:1px solid #374151;border-radius:4px;max-height:240px;overflow-y:auto;width:min(320px,90vw);box-shadow:0 8px 24px rgba(0,0,0,0.4);"></div>';
if (html.includes(buttonAnchor) && !html.includes('id="keyword-dropdown"')) {
  html = html.replace(buttonAnchor, floatingDropdown);
  changes++;
  console.log("✓ Dropdown re-inserted as floating overlay right after Keywords button");
} else if (html.includes('id="keyword-dropdown"')) {
  console.log("⚠ A keyword-dropdown div already exists somewhere — skipping insert to avoid duplicates");
} else {
  console.log("⚠ Button anchor not found, skipped");
}

if (changes === 3) {
  fs.writeFileSync('index.html.pre-dropdown-fix-v2-backup', backup);
  fs.writeFileSync('index.html', html);
  console.log("\n3/3 changes applied. Backup: index.html.pre-dropdown-fix-v2-backup");
} else {
